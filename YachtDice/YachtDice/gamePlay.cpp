...
void run_yahtzee_game() {
    ...
    writeAt(5, 11, u8"4. 종료");
    writeAt(5, 13, u8"원하는 모드의 번호를 입력하세요: ");
    while (!(cin >> gameMode) || gameMode < 1 || gameMode > 4) {
        cin.clear(); cin.ignore((numeric_limits<streamsize>::max)(), '\n');
        writeAt(5, 14, string(40, ' '));
        writeAt(5, 14, u8"1~4 사이의 올바른 숫자를 입력하세요: ");
    }
    ...
    int y = 4;
    for (const auto& pl : players) writeAt(5, y++, fitName(pl.name, 15) + " : " + to_string(pl.total()) + u8"점");
    writeAt(5, y + 2, u8"'" + players[0].name + u8"' 님의 승리입니다!");
    writeAt(5, y + 4, u8"아무 키나 눌러 종료하세요...");
    cin.get();
